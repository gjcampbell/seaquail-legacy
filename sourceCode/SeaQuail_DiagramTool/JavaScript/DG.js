    ...
    this.MyMouseUp = function(e) {
        ...
        if (fullClick && (e.Button == MseButtons.Left || e.Button == MseButtons.Right)) {
            ...
            if (e.Button == MseButtons.Left) {
                ...
                e.DisableContextMenu();
                var items = [];
                items.push(new MenuItem(SSMSIcons.ColumnAdd(), 'Add Column'));
                if (_Parent.SelectedColumn != null) {
                    items.push(new MenuItem(SSMSIcons.ColumnInsert(), 'Insert Column'));
                    items.push(new MenuItem(SSMSIcons.ColumnDelete(), 'Delete Column'));
                    if (_Columns()[_Parent.SelectedColumn].DataType.FKOK) {
                        items.push(new MenuItem(FFFIcons.Relationship(), 'Set Foreign Key'));
                    }
                }
...
function DGDiagram(canvas) {
    ...
    var _CreateMouseEvent = function(e) {
        return { Event: e, Pos: _MsePos, Button: e.which, DisableContextMenu: _DoPreventContextMenu };
    }
    ...
    var cvs_MouseWheel = function(e, delta) {
        if (!_DampenWheel) {
            ...
            self.SetOffset(new Pt(-_MsePos.X + (self.GetW() / 2) / self.GetZoom(), -_MsePos.Y + (self.GetH() / 2) / self.GetZoom()));
            _DampenWheel = true;
