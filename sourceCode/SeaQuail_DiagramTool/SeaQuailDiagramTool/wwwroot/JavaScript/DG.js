...
var SSMSIcons = {
    ColumnAdd: function() { return this.GetURL('column_add'); },
    ColumnInsert: function() { return this.GetURL('column_insert'); },
    ColumnDelete: function() { return this.GetURL('column_delete'); },
    GetURL: function(ssmsicon) {
        return '/Media/Images/Icons/SSMS/' + ssmsicon + '.png';
    }
}
var FFFIcons = {
    Relationship: function() { return this.GetURL('table_relationship'); },
    GetURL: function(ssmsicon) {
        return '/Media/Images/Icons/' + ssmsicon + '.png';
    }
}

var ZoomLevels = [.25, .33, .50, .75, .87, 1, 1.25, 1.5, 2];

var _DataTypes = {
    Boolean: { Name: 'Boolean', HasLength: false, HasPrecision: false, IDOK: false, PKOK: false, FKOK: false },
    SmallInt: { Name: 'SmallInt', HasLength: false, HasPrecision: false, IDOK: true, PKOK: true, FKOK: true },
    Int: { Name: 'Int', HasLength: false, HasPrecision: false, IDOK: true, PKOK: true, FKOK: true },
    BigInt: { Name: 'BigInt', HasLength: false, HasPrecision: false, IDOK: true, PKOK: true, FKOK: true },
    Decimal: { Name: 'Decimal', HasLength: false, HasPrecision: true, IDOK: false, PKOK: true, FKOK: true },
    DateTime: { Name: 'DateTime', HasLength: false, HasPrecision: false, IDOK: false, PKOK: false, FKOK: false },
    String: { Name: 'String', HasLength: true, HasPrecision: false, IDOK: false, PKOK: true, FKOK: true },
    Bytes: { Name: 'Bytes', HasLength: true, HasPrecision: false, IDOK: false, PKOK: false, FKOK: false },
    GetDataTypeName: function(col) {
        return col.DataType.Name +
            ((col.DataType.HasPrecision) ? '(' + col.Precision + ', ' + col.Scale + ')'
            : (col.DataType.HasLength) ? '(' + (col.Length == 0 ? 'MAX' : col.Length) + ')'
            : '');
    },
    Validation: new RegExp(/^Boolean$|^SmallInt$|^Int$|^BigInt$|^Decimal\(\d+,[ ]?\d+\)$|^DateTime$|^(String|Bytes)\((\d+|max)\)$/i),
...
function DGDiagramObject(parent) {
    ...
    this.DG = function() { return this.GetParent().DG(); }
}
...
function DGTable(parent) {
    ...
    this.GetType = function() { return 'DGTable'; }
    var _Parent = parent,
        self = this,
        _Highlight = false;
    ...
    this.MyMouseDown = function(e) {
        if (e.Button == MseButtons.Left) {
            dragging = { MyStartPos: { X: this.GetX(), Y: this.GetY() }, MseStartPos: e.Pos };
            e.Cancel = true;
            this.Select();
        }
    }
    ...
    this.MyDraw = function(ctx) {
        ...
        fillColor = this.DG().SelectedTable != this ? fillColor : this.Group ? penColor : '#1C4B7F';
        ...
        if (_Highlight)
            ctx.Highlight(true);
        ctx.DrawRoundedRect(new Rect(dim.X, dim.Y, dim.W, 31), 7, new Pen(penColor, 1));
        ctx.Highlight(false);
        ctx.FillText(_Name, { X: dim.X + 12, Y: dim.Y + 20 }, (_Parent.SelectedTable == this ? '#ffffff' : this.Group ? penColor : '#000000'), new Glyph('bold 9pt Arial', null, null, dim.W - 12));
    }

    var _Name = '';
    var _Columns = [
        { Name: '', DataType: _DataTypes.String, Length: 250, Nullable: true, GUID: guid() }
    ];

    this.SetName = function(n) { _Name = n; };
    this.GetName = function() { return _Name; };
    this.GetGUID = function() { return _Guid; };
    this.SetGUID = function(guid) { _Guid = guid; };
    this.GetColumns = function() { return _Columns; };
    this.GetColList = function() { return _ColList; };
    ...
    this.Delete = function() {
        if (this.DG().SelectedTable == this)
            this.DG().SelectedTable = null;
...
function DGColList(parent) {
    DGDiagramObject.call(this, parent);
    this.GetType = function() { return 'DGColList'; }
    var _Parent = parent,
        self = this,
        _initialDraw = true;

    var _Columns = function() { return _Parent.GetColumns(); };

    var _RowHeight = 24,
        _BorderPen = new Pen('#cfcfcf', .5),
        _CellGlyph = new Glyph('8pt Arial', null, TextBaselines.Middle),
        _HeaderCellGlyph = new Glyph('8pt Arial', TextAligns.Center, TextBaselines.Middle),
        _CellGrad = null,
        _HighlightIndexes = [];        
    ...
    var _CreateColumn = function() {
        return { Name: '', DataType: _DataTypes.String, Length: 250, Nullable: true, GUID: guid() };
    }

    var _CreateRowHighlight = function(ctx, x, y) {
        return ctx.CreateLinearGrad(new LinearGrad(x, y, x, y + _RowHeight, [
            new ColorStop(0, '#729844'),
            new ColorStop(.2, '#BAD698'),
            new ColorStop(.8, '#BAD698'),
    ...
    var _CreateRowSelHighlight = function(ctx, x, y) {
        return ctx.CreateLinearGrad(new LinearGrad(x, y, x, y + _RowHeight, [
            ...
            new ColorStop(.2, '#9DB7D1'),
            new ColorStop(.8, '#9DB7D1'),
    ...
    var _DrawRow = function(ctx, data, rowidx, x, y) {
        ...
        if (self.DG().GetCurrentFK() != null) {
            ...
            overrideBG = thecol.DataType == fromcol.DataType && thecol != fromcol ? _CreateRowHighlight(ctx, x, y) : '#F1F2F4';
        }
        for (var i in _ColStyles) {
            var col = _ColStyles[i];
            var rect = new Rect(x, y, col.Width, _RowHeight);
            ctx.FillRect(rect, (i == 0 ? col.BG : _Parent.SelectedColumn == rowidx ? _CreateRowSelHighlight(ctx, x, y) : overrideBG ? overrideBG : col.BG));
            if (i == 1 && _HighlightIndexes.indexOf(rowidx) > -1)
                ctx.Highlight(true);
            ctx.DrawRect(rect, _BorderPen);
            ctx.Highlight(false);
            _CellGlyph.Align = col.Align;
            var textX = _CellGlyph.Align == TextAligns.Left ? x + 3
                : _CellGlyph.Align == TextAligns.Center ? x + (col.Width / 2)
                : col.Width - 3 + x;
            _CellGlyph.Width = col.Width - 3;
            ctx.FillText(data[i], new Pt(textX, y + (_RowHeight / 2)), '#000000', _CellGlyph);
    ...
    var _DrawHeader = function(ctx, x, y) {
        for (var i in _ColStyles) {
            ...
            ctx.FillText(col.Name, new Pt(textX, y + (_RowHeight / 2)), '#000000', _HeaderCellGlyph);
    ...
    this.Select = function(index) { _Parent.SelectedColumn = index; }
    this.Deselect = function() { _Parent.SelectedColumn = null; this.Invalidate(); }
    ...
    this.GetRowHeight = function() { return _RowHeight; }
    ...
    this.MyMouseMove = function(e) {
        if (_DownPos != null) {
            if (!_DownPos.Equal(e.Pos)
                ...
                if (col.DataType.FKOK)
                    this.DG().AddRelationship(_Parent, col);
            }
        }
    }
    ...
    var ContextMenu_Click = function(args) {
        if (args.Text == 'Insert Column') {
            ...
            _Parent.GetParent().AddRelationship(_Parent, _Columns()[_Parent.SelectedColumn]);
        } else if (args.Text == 'Remove Foreign Key') {
            _Parent.GetParent().RemoveRelationship(_Columns()[_Parent.SelectedColumn]);
        }
    ...
    var _GetNextEditCell = function(dir) {
        ...
        for (var row = _EditIndex.Row; row < _Columns().length && row >= 0; row += dir) {
            for (var col = startCol; col < _ColStyles.length && col >= 0; col += dir) {
    ...
    var _EditColumnText = function(colIndex, rowIndex, suggest) {
        ...
        // position
            new Rect(
                ...
                (pos.Y + (rowIndex + 1) * _RowHeight) + 1 + (dg.GetOffsetY()) + $(self.DG().TheCanvas()).offset().top,
                colStyle.Width,
                _RowHeight),
        // styles
    ...
    this.GetEditIndex = function() { return _EditIndex; }
    this.GetColStyles = function() { return _ColStyles; }

    this.MyMouseUp = function(e) {
        var fullClick = _DownPos != null && _DownPos.X == e.Pos.X && _DownPos.Y == e.Pos.Y;

        if (fullClick && (e.Button == MseButtons.Left || e.Button == MseButtons.Right)) {
            var pos = this.GetPos();
            ...
            if (e.Button == MseButtons.Left) {
                ...
                if (_Parent.SelectedColumn != null) {
                    ...
                    if (_Columns()[_Parent.SelectedColumn].DataType.FKOK) {
                        ...
                        if (_Parent.GetParent().GetRelationshipByFromColumn(_Columns()[_Parent.SelectedColumn])) {
                            items.push(new MenuItem('unlink', 'Remove Foreign Key'));
                        }
                    }
                }

                ContextMenu.Show(new Pt(e.Event.pageX, e.Event.pageY), items, ContextMenu_Click);
            }
            e.Cancel = true;
        }
...
function DGGroup(parent) {
    DGDiagramObject.call(this, parent);
    this.GetType = function() { return 'DGGroup'; };
    ...
    var _Parent = parent,
        ...
        _PtInEllipse = function(pt) {
            var c = self.GetCenter(), rx = self.GetW() / 2, ry = self.GetH() / 2,
                dist = (Math.pow(pt.X - c.X, 2) / Math.pow(rx, 2))
        ...
        _StartDragging = function(pos, handle) {
            ...
            if (handle == null) {
                if (_GetDetachHandlePt().Distance(pos) > 15) {
                    ...
                    _Dragging.Tables = [];
                    for (var i in tables) {
                        _Dragging.Tables.push({ Table: tables[i], StartPos: new Pt(tables[i].GetX(), tables[i].GetY()) });
                    }
                }
            }
        },
                ...
                if (_Dragging.Handle == null) {
                    ...
                    switch (_Handles[_Dragging.Handle]) {
                        case 'Right':
                            ...
                            break;
                        case 'Left':
                            self.SetW(_Dragging.MyStartSize.W - (e.Pos.X - _Dragging.MseStartPos.X));
                            self.SetX(newX);
                            break;
                        case 'Top':
                            self.SetH(_Dragging.MyStartSize.H - (e.Pos.Y - _Dragging.MseStartPos.Y));
                            self.SetY(newY);
                            break;
                        case 'Bottom':
    ...
    this.Draw = function(ctx) {
        var children = this.DG().GetChildren(),
            dhr = _DragHandleRadius,
            r = new Rect(this.GetX() + dhr, this.GetY() + dhr, this.GetW() - (2 * dhr), this.GetH() - (2 * dhr));
        ctx.ctx().save();
    ...
    this.MyMouseDown = function(e) {
        if (e.Button == MseButtons.Left) {
            var dhs = _GetDragHandlePts(), c = this.GetCenter();
            // check mousedown on a drag handle
            for (var i = 0; i < dhs.length; i++) {
                if (Math.sqrt(Math.pow(dhs[i].Y - e.Pos.Y, 2) + Math.pow(dhs[i].X - e.Pos.X, 2)) < _DragHandleRadius) {
            ...
            if (_Dragging == null && _PtInEllipse(e.Pos))
                _StartDragging(e.Pos, null);
...
function DGFKey(parent) {
    DGDiagramObject.call(this, parent);
    this.GetType = function() { return 'DGFKey'; }
    var _Parent = parent;
    var self = this;
    ...
    var _GetColPos = function(tbl, col) {
        if (tbl != null) {
            var tPos = tbl.GetPos();
            tPos.Y += ((tbl.GetColumns().indexOf(col) + 1) * tbl.GetColList().GetRowHeight()) + tbl.GetColList().GetY() + (tbl.GetColList().GetRowHeight() / 2);
            tPos.X += 7;
    ...
    this.GetFromTable = function() { return _FromTable; }
    this.GetFromColumn = function() { return _FromCol; }
    this.GetToTable = function() { return _ToTable; }
    this.GetToColumn = function() { return _ToCol; }
    ...
    this.Draw = function(ctx) {
        if (this.DG().GetCurrentFK() != this) {
            if (this.DG().GetChildren().indexOf(this.GetFromTable()) < 0
        ...
        var endRight = (_ToTable != null) ? new Pt(endLeft.X + _ToTable.GetW() - 14, endLeft.Y) : endLeft;
        var closest = [[startLeft, endLeft], [startLeft, endRight], [startRight, endLeft], [startRight, endRight]];
        ...
        if (_ToTable != null) {
            ...
            if (startOff == endOff)
                bezPt1.X = startPos.X + (Math.abs(startPos.X - endPos.X) + 30) * startOff;
            if (startOff == endOff)
        ...
        c.rect(startOff == -1 ? startPos.X - 7 : startPos.X - 1, startPos.Y - 3, 8, 6);
        c.strokeStyle = '#9A9A9A';
        c.lineWidth = 1;
        c.stroke();
        c.fillStyle = '#e6e6e6';
        ...
        c.arc(endPos.X, endPos.Y, 6, Math.PI * .5, Math.PI * 1.5, endOff == 1);
        c.fillStyle = '#9A9A9A';
        c.fill();
        c.closePath(); // end ---------        
...
function DGDiagram(canvas) {
    ...
    var _ChildrenSort = function(a, b) {
        return b.GetType() == a.GetType() ? 0 : _ChildZOrder[a.GetType()] > _ChildZOrder[b.GetType()] ? 1 : -1;
    }
    ...
    this.FindChildren = function(lambda) {
        var res = [], children = this.GetChildren();
    ...
    this.SetDragMode = function(ison) { _DragMode = ison; }
    this.GetDragMode = function(ison) { return _DragMode; }

    this.GetZoom = function() { return _Zoom; };
    this.GetZoomIdx = function() { return _ZoomIndex; }
    this.SetZoomIdx = function(idx) {
        _ZoomIndex = idx;
        $(_CurrentZoom).stop().animate({ zoom: ZoomLevels[idx] }, {
            duration: 600,
            step: function () {
    ...
    this.ZoomOut = function() { this.SetZoomIdx(_ZoomIndex - 1 >= 0 ? _ZoomIndex - 1 : 0); }
    this.ZoomIn = function() { this.SetZoomIdx(_ZoomIndex + 1 < ZoomLevels.length ? _ZoomIndex + 1 : _ZoomIndex); }
    this.Reset = function() {
        this.SetZoomIdx(5);
    ...
    this.GetOffsetX = function() { return _Offset.X; }
    this.GetOffsetY = function() { return _Offset.Y; }
    ...
    this.GetView = function() {
        return new Rect(-this.GetOffsetX(), -this.GetOffsetY(), this.GetW() / this.GetZoom(), this.GetH() / this.GetZoom());
    }
    
    this.GetName = function() { return _Name; }
    this.SetName = function(name) { _Name = name; }

    this.TheCanvas = function() { return _Cvs; }

    var _MsePos = null;

    this.GetMousePos = function() { return _MsePos; }
    ...
    this.UpdateTableGroups = function(groups, tables) {
        var groups = groups || this.FindChildren(function(o) { return o.GetType() == 'DGGroup'; }),
            tables = tables || this.FindChildren(function(o) { return o.GetType() == 'DGTable'; });
        for (var i in tables) {
            var t = tables[i], tc = t.GetCenter(), found = false
    ...
    this.GetLastGroupIndex = function() {
        var grps = this.FindChildren(function(o) { return o.GetType() == 'DGGroup'; });
        return grps.length - 1;
    }

    this.GetLastFKeyIndex = function() {
        var fkeys = this.FindChildren(function(o) { return o.GetType() == 'DGFKey'; });
        return fkeys.length - 1 + this.GetLastGroupIndex();
    }

    this.GetCurrentFK = function() { return _CurrentFK; }

    // Canvas event handlers
    var _DoPreventContextMenu = function() { _PreventContextMenu = true; }
    ...
    var _CreateMouseEvent = function(e) {
        const button = !!e.which ? e.which : typeof e.button === 'number' ? e.button + 1 : 1;
        return { Event: e, Pos: _MsePos, Button: button, DisableContextMenu: _DoPreventContextMenu };
    }
    ...
    var doc_MouseMove = function(e) {
        ...
        if (_MScroll != null) {
            _ScrollVel.X = Math.max(Math.min((_MScroll.StartPos.X - e.pageX) / 60, 2), -2);
            _ScrollVel.Y = Math.max(Math.min((_MScroll.StartPos.Y - e.pageY) / 60, 2), -2);
        }
    }
    $(document).mousemove(doc_MouseMove);

    var cvs_MouseMove = function(e) {
        _MsePos = new Pt(e.pageX / _Zoom - _Offset.X, (e.pageY - _YOffset) / _Zoom - _Offset.Y);
    ...
    $(_Cvs).mousemove(cvs_MouseMove).mouseup(cvs_MouseUp).mousedown(cvs_MouseDown)
    ...
    var cvs_MouseWheel = function(e, delta) {
        if (!_DampenWheel) {
            ...
            $(_Offset).animate(new Pt(-_MsePos.X + (self.GetW() / 2) / self.GetZoom(), -_MsePos.Y + (self.GetH() / 2) / self.GetZoom()), {
                duration: 600,
                step: function () {
            ...
            _DampenWheel = true;
            setTimeout(function() { _DampenWheel = false; }, 100);
        }
    }
    ...
    this.Draw = function(ctx) {
        if (_DragMode) {
            this.SetOffset(new Pt(_Offset.X + _ScrollVel.X * 30, _Offset.Y + _ScrollVel.Y * 30))
        }

        _Ctx.ctx().clearRect(0 - _Offset.X, 0 - _Offset.Y, this.GetW() / _Zoom, this.GetH() / _Zoom);
